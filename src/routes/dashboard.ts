  ...
  try {
    const ultimasConversaciones = await db('conversaciones')
      .select(
        ...
        db.raw("CASE WHEN conversaciones.estado = 'CERRADA' AND conversaciones.cerrada_en IS NOT NULL THEN EXTRACT(EPOCH FROM (conversaciones.cerrada_en - COALESCE(conversaciones.asignada_en, conversaciones.creada_en)))::int ELSE NULL END AS segundos_duracion"),
